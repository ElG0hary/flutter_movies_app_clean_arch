...
class MovieRemoteDataSourceImpl extends BaseMovieRemoteDataSource {
  ...
  Future<List<MovieModel>> getNowPlayingMovies() async {
    final response = await Dio().get(ApiConstants.NOW_PLAYING_MOVIES);
    return _dataOrException(response);
  }

  @override
  Future<List<MovieModel>> getPopularMovies() async {
    final response = await Dio().get(ApiConstants.POPULAR_MOVIES);
    return _dataOrException(response);
  }

  @override
  Future<List<MovieModel>> getTopRatedMovies() async {
    final response = await Dio().get(ApiConstants.TOP_RATED_MOVIES);
    return _dataOrException(response);
  }
